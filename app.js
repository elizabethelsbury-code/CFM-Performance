...
async function handleAuthSubmit(){
  ...
  if(authMode === 'login'){
    ...
    // CONFIRMED: profiles(id, role, full_name, created_at)
    const { error: profErr } = await supabaseClient.from('profiles').insert({ id: data.user.id, role: 'client', full_name: name });
...
async function loadGoals(){
  const { data, error } = await supabaseClient.from('goals').select('*').eq('client_id', currentUser.id);
  if(error){ goals = []; return; }
  goals = data || [];
}

async function loadLifestyleLogs(){
  const { data, error } = await supabaseClient.from('lifestyle_logs').select('*').eq('client_id', currentUser.id).order('date', { ascending: false });
...
async function saveVideoUrl(){
  ...
  const { error } = await supabaseClient.from('plans').upsert({ client_id: currentUser.id, plan_data: plan }, { onConflict: 'client_id' });
...
async function loadClientDetail(clientId){
  const [{ data: planRow }, { data: sessionRows }, { data: goalRows }, { data: lifestyleRows }] = await Promise.all([
    supabaseClient.from('plans').select('plan_data').eq('client_id', clientId).maybeSingle(),
    supabaseClient.from('sessions').select('*').eq('client_id', clientId).order('date', { ascending: false }).limit(10),
    supabaseClient.from('goals').select('*').eq('client_id', clientId),
    supabaseClient.from('lifestyle_logs').select('*').eq('client_id', clientId).order('date', { ascending: false }).limit(7),
  ]);
  clientDetail = {
...
function renderClientDetail(){
  ...
  main.innerHTML = `
    ...
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
        ...
        <div style="display:flex;gap:10px;">
          <button id="importProgramBtn" style="background:none;border:none;color:#6E93AC;font-family:var(--font-mono);font-size:11px;">IMPORT FROM NOTES</button>
          <button id="editProgramBtn" style="background:none;border:none;color:var(--brass);font-family:var(--font-mono);font-size:11px;">EDIT PROGRAM</button>
        </div>
      </div>
  ...
  document.getElementById('editProgramBtn').onclick = ()=>renderProgramEditor(main);
...
async function loadSettings(){
  const { data, error } = await supabaseClient.from('settings').select('*').eq('client_id', currentUser.id).maybeSingle();
...
async function saveStartDate(newDate){
  const { error } = await supabaseClient.from('settings').upsert({ client_id: currentUser.id, start_date: newDate }, { onConflict: 'client_id' });
  if(error){ showToast("Couldn't save — " + error.message); return; }
...
async function loadCycleEntries(){
  const { data, error } = await supabaseClient.from('cycle_entries').select('*').eq('client_id', currentUser.id).order('date', { ascending: false });
...
function wireCycleView(main){
  ...
  document.getElementById('saveCycleBtn').onclick = async ()=>{
    ...
    const row = {
      client_id: currentUser.id, date: cycleState.date, flow: cycleState.flow, period_start: cycleState.periodStart,
...
async function saveClinicianPlan(){
  const { error } = await supabaseClient.from('plans').upsert(
    { client_id: selectedClientId, plan_data: clientDetail.plan },
...
function ensureClinicianDay(dayKey){
  if(!clientDetail.plan[dayKey]) clientDetail.plan[dayKey] = { type:'rest', label: dayKey, desc:'', exercises:[] };
  return clientDetail.plan[dayKey];
}
...
function wireProgramEditor(main){
  document.getElementById('doneEditingBtn').onclick = async ()=>{
    ...
    if(ok){ showToast('Program saved'); renderClientDetail(); }
  };
  ...
  main.querySelectorAll('.exNameEdit').forEach(inp=>{ inp.oninput = e=>{ clientDetail.plan[inp.dataset.day].exercises[inp.dataset.exidx].name = e.target.value; }; });
  main.querySelectorAll('.exTargetEdit').forEach(inp=>{ inp.oninput = e=>{ clientDetail.plan[inp.dataset.day].exercises[inp.dataset.exidx].target = e.target.value; }; });
  main.querySelectorAll('.heroToggleEdit').forEach(btn=>{
    btn.onclick = ()=>{
      const day = btn.dataset.day, idx = btn.dataset.exidx;
      clientDetail.plan[day].exercises[idx].hero = !clientDetail.plan[day].exercises[idx].hero;
      renderProgramEditor(main);
    };
  });
  main.querySelectorAll('.removeExEdit').forEach(btn=>{
    btn.onclick = ()=>{
      ...
      clientDetail.plan[day].exercises.splice(idx,1);
      renderProgramEditor(main);
    };
  });
...
function renderClinicianLibraryPicker(main, dayKey){
  ...
  function renderResults(filter){
    ...
    resultsEl.querySelectorAll('.clinLibExerciseBtn').forEach(b=>{
      b.onclick = ()=>{
        clientDetail.plan[dayKey].exercises.push({ name: b.dataset.name, target:'', hero:false });
        renderProgramEditor(main);
      };
    });
  }
...
Leg curl 3x10-12" style="width:100%;background:var(--surface-2);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:10px;font-family:var(--font-mono);font-size:13px;margin-bottom:12px;box-sizing:border-box;"></textarea>
  ...
  `;
  document.getElementById('cancelProgramImportBtn').onclick = ()=>renderClientDetail();
...
function renderProgramImportPreview(main){
    ...
    html += `<div class="card">
      ...
      <div class="field" style="max-width:260px;"><label>Import into</label>
        <select class="programImportDaySelect" data-idx="${i}" style="width:100%;background:var(--surface-2);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:9px;font-size:13px;">
          ...
          ${DAY_ORDER.map(d=>`<option value="${d}" ${blk.dayKey===d?'selected':''}>${d} — currently: ${escapeAttr(clientDetail.plan[d].label)}</option>`).join('')}
        </select>
      </div>
    </div>`;
  });
  ...
  document.getElementById('cancelProgramImportBtn2').onclick = ()=>renderClientDetail();
  document.getElementById('commitProgramImportBtn').onclick = async ()=>{
    programImportPreview.forEach(blk=>{
      if(!blk.dayKey) return;
      clientDetail.plan[blk.dayKey] = { type:'lift', label: blk.title, exercises: blk.exercises };
    });
    const ok = await saveClinicianPlan();
    if(ok){ showToast('Program imported'); renderClientDetail(); }
  };
}
