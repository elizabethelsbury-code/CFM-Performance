...
function renderLog(main){
  ...
  main.innerHTML = `
    ...
    <div class="field"><label>Logging as</label>
      <select id="logDaySelect" style="width:100%;background:var(--surface-2);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:11px;font-size:15px;">
        ${liftDayKeys.map(k=>`<option value="${escapeAttr(k)}" ${k===logState.dayKey?'selected':''}>${escapeHtml(plan[k].label||k)}</option>`).join('')}
        ${liftDayKeys.length===0 ? `<option value="${escapeAttr(logState.dayKey)}" selected>${escapeHtml(logState.dayKey)} (no program set)</option>` : ''}
      </select>
    </div>
    <div class="section-label">${formatAU(logState.date)}</div>
  ...
  `;
  document.getElementById('logDaySelect').onchange = e=>{
    logState.dayKey = e.target.value;
...
function renderSchedule(main){
  ...
  if(trackCycleBtn){
    trackCycleBtn.onclick = ()=>saveTrackCycle(!settings.trackCycle);
  }
...
function renderProgramEditor(main){
  ...
  html += `<div class="section-label" style="margin-top:20px;">Flexible / isolated training days</div>
    <div style="font-size:12px;color:var(--text-dim);margin-bottom:10px;">Not tied to a day of the week — the client picks one of these when logging, whatever day they actually train.</div>`;
