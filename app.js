...
let logState = { date: todayISO(), dayKey: todayDayName(), exercises: [], warmup: '', note: '', cardioNote: '' };
...
function showToast(msg){
  ...
  if(!t){
    ...
    t.id = 'toast';
    t.style.cssText = 'position:fixed;bottom:calc(80px + env(safe-area-inset-bottom));left:50%;transform:translateX(-50%);background:var(--surface-2);border:1px solid var(--border);color:var(--text);padding:10px 16px;border-radius:8px;font-size:13px;z-index:300;opacity:0;transition:opacity .2s;pointer-events:none;';
...
function presetSets(count){
  return Array.from({ length: count }, ()=>({ weight:'', reps:'', rir:'' }));
}
...
function renderLog(main){
  ...
  const day = plan[logState.dayKey];
  const liftDayKeys = Object.keys(plan).filter(k=>plan[k].type==='lift' || plan[k].type==='cardio');
  ...
  main.innerHTML = `
    ...
    <div style="display:flex;gap:10px;margin-bottom:14px;">
      <div class="field" style="flex:1;margin-bottom:0;"><label>Date</label><div class="dateinput-wrap"><input type="date" id="logDateInput" value="${logState.date}"></div></div>
    ...
    ${day && day.type === 'cardio' ? `
      <div class="field">
        <label>Cardio</label>
        <textarea id="logCardioInput" rows="3" placeholder="e.g. Bike 20 min moderate, Rowing erg 2km, Stepper 15 min">${escapeAttr(logState.cardioNote||'')}</textarea>
      </div>
    ` : `
    <div class="section-label" style="margin-top:10px;">Exercises</div>
    ${findLastSessionForDayKey(logState.dayKey, logState.date) ? `<button class="btn secondary" id="copyLastWorkoutBtn" style="margin-bottom:10px;">Copy last "${escapeAttr((day&&day.label)||logState.dayKey)}" workout</button>` : ''}
    <div id="logSaveErr" class="err" style="display:none;"></div>
    <div id="logExerciseList"></div>
    <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:6px;">
      <button class="btn secondary" id="addCustomExBtn" style="flex:1;min-width:140px;">+ Add exercise</button>
      ${missingProgramExercises(day).length > 0 ? `<button class="btn secondary" id="addRemainingExBtn" style="flex:1;min-width:140px;">+ Add remaining (${missingProgramExercises(day).length})</button>` : ''}
    </div>
    `}
  ...
  const cardioInput = document.getElementById('logCardioInput');
  if(cardioInput) cardioInput.oninput = e=>{ logState.cardioNote = e.target.value; };
  document.getElementById('logNoteInput').oninput = e=>{ logState.note = e.target.value; };
  renderLogExerciseList();
  const addExBtn = document.getElementById('addCustomExBtn');
  if(addExBtn) addExBtn.onclick = openAddExerciseFlow;
  const addRemainingBtn = document.getElementById('addRemainingExBtn');
  if(addRemainingBtn) addRemainingBtn.onclick = ()=>openAddRemainingFlow(main);
  const copyLastBtn = document.getElementById('copyLastWorkoutBtn');
  if(copyLastBtn) copyLastBtn.onclick = ()=>copyLastDaysWorkout(main);
  ensureLogSaveBar();
  renderRestTimerBar();
}

// ---------- LAST TIME REFERENCE (projected weights) ----------
// Dumbbells are commonly stocked in 1kg increments up to 10kg, then 2.5kg
// increments beyond that. Barbells/cables/machines load in 2.5kg plates
// throughout, so they always round to the nearest 2.5kg.
function isDumbbellExercise(name){
  const n = (name||'').toLowerCase();
  return n.includes('db') || n.includes('dumbbell');
}
function roundWeightForExercise(name, rawWeight){
  if(rawWeight == null || isNaN(rawWeight)) return null;
  if(isDumbbellExercise(name)){
    const increment = rawWeight <= 10 ? 1 : 2.5;
...
function getLastLoggedExercise(name){
  ...
  for(const s of sorted){
    ...
    if(ex && ex.sets && ex.sets.length){
      if(getWeekAndPhase(s.date).deload){
        if(!deloadFallback) deloadFallback = { date: s.date, sets: ex.sets };
        continue; // skip deload sessions for now — they're deliberately lighter, not representative
      }
      return { date: s.date, sets: ex.sets };
    }
  }
  return deloadFallback; // only fall back to a deload session if there's truly no other history
}
function lastTimeReference(name){
  const last = getLastLoggedExercise(name);
  if(!last) return null;
  const text = last.sets.map(s=>`${s.weight||'–'}kg × ${s.reps||'–'}${s.rir?' @ RIR '+s.rir:''}`).join(', ');
...
function deloadSuggestionForExercise(name){
  const last = getLastLoggedExercise(name); // already deload-aware — skips deload-week sessions
  if(!last) return null;
  const weights = last.sets.map(s=>parseFloat(s.weight)).filter(n=>!isNaN(n));
  if(!weights.length) return null;
  const topWeight = Math.max(...weights);
  return roundWeightForExercise(name, topWeight * 0.65);
}
const RIR_OPTIONS = ['0-1','1-2','3-4','Failure'];
function renderLogExerciseList(){
  ...
  const isDeloadWeek = getWeekAndPhase(logState.date).deload;
  el.innerHTML = logState.exercises.map((ex, exIdx)=>{
    const lastTime = ex.name && ex.name.trim() ? lastTimeReference(ex.name) : null;
    const deloadSuggestion = (isDeloadWeek && ex.name && ex.name.trim()) ? deloadSuggestionForExercise(ex.name) : null;
    const isFirst = exIdx === 0;
    const isLast = exIdx === logState.exercises.length - 1;
    return `
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">
        ...
        <div style="display:flex;align-items:center;gap:6px;flex-shrink:0;">
          <button class="moveExUpBtn" data-idx="${exIdx}" ${isFirst?'disabled':''} style="background:none;border:none;color:${isFirst?'var(--text-faint)':'var(--text-dim)'};font-size:16px;opacity:${isFirst?'0.35':'1'};padding:2px 4px;">&#9650;</button>
          <button class="moveExDownBtn" data-idx="${exIdx}" ${isLast?'disabled':''} style="background:none;border:none;color:${isLast?'var(--text-faint)':'var(--text-dim)'};font-size:16px;opacity:${isLast?'0.35':'1'};padding:2px 4px;">&#9660;</button>
          <button class="removeExBtn" data-idx="${exIdx}" style="background:none;border:none;color:var(--text-faint);font-size:16px;padding:2px 4px;">&times;</button>
        </div>
      </div>
      ${deloadSuggestion !== null ? `<div style="background:var(--sage-dim);border:1px solid var(--sage-dim);border-radius:8px;padding:8px 10px;margin-bottom:10px;font-size:12px;color:var(--sage);line-height:1.4;">
        <span style="font-family:var(--font-mono);font-size:10px;text-transform:uppercase;letter-spacing:0.04em;">Deload week · try ~65%</span><br>
        ${deloadSuggestion}kg
      </div>` : ''}
      ...
      <div class="setrows" data-idx="${exIdx}">
        ${ex.sets.map((s,setIdx)=>`
          <div class="setrow">
            ...
            <select class="rirSelect" data-idx="${exIdx}" data-set="${setIdx}">
  ...
  el.querySelectorAll('.moveExUpBtn').forEach(b=>b.onclick = ()=>{
    const idx = +b.dataset.idx;
    if(idx <= 0) return;
    [logState.exercises[idx-1], logState.exercises[idx]] = [logState.exercises[idx], logState.exercises[idx-1]];
    renderLogExerciseList();
  });
  el.querySelectorAll('.moveExDownBtn').forEach(b=>b.onclick = ()=>{
    const idx = +b.dataset.idx;
    if(idx >= logState.exercises.length - 1) return;
    [logState.exercises[idx], logState.exercises[idx+1]] = [logState.exercises[idx+1], logState.exercises[idx]];
    renderLogExerciseList();
  });
  ...
  el.querySelectorAll('.addSetBtn').forEach(b=>b.onclick = ()=>{
    logState.exercises[+b.dataset.idx].sets.push({ weight:'', reps:'', rir:'' });
    renderLogExerciseList();
  });
  ...
  el.querySelectorAll('.rirSelect').forEach(sel=>{
    sel.onchange = ()=>{
      logState.exercises[+sel.dataset.idx].sets[+sel.dataset.set].rir = sel.value;
    };
  });
}

// Program exercises for today's day that aren't currently in the log, by
// name. Computed fresh each time it's called — never auto-adds anything on
// its own, just tells the caller what's missing so a person can explicitly
// choose what to add via a checklist.
// Most recent OTHER session logged against this same program day slot —
// used by "Copy last workout" to pull in what was actually done last time
// (including any swaps), not just the generic program template.
function findLastSessionForDayKey(dayKey, excludeDate){
  const matches = sessions.filter(s=>s.day_key===dayKey && s.date!==excludeDate).sort((a,b)=>b.date.localeCompare(a.date));
  return matches[0] || null;
}
function applyCopiedWorkout(main, last){
  logState.exercises = (last.exercises||[]).map(e=>({
    id: uid(), name: e.name, target: e.target||'', videoUrl: videoUrlForExerciseName(e.name),
    sets: presetSets((e.sets||[]).length || parseSetCountFromTarget(e.target)),
  }));
  logState._initedForPlan = true;
  renderLog(main);
  showToast(`Copied ${last.exercises.length} exercise${last.exercises.length!==1?'s':''} from ${formatAU(last.date)}`);
}
function copyLastDaysWorkout(main){
  const last = findLastSessionForDayKey(logState.dayKey, logState.date);
  if(!last) return;
  const hasEnteredData = logState.exercises.some(e=>(e.sets||[]).some(s=>s.weight!=='' || s.reps!==''));
  if(!hasEnteredData){
    applyCopiedWorkout(main, last);
    return;
  }
  let ov = document.getElementById('copyWorkoutConfirmOverlay');
  if(ov) ov.remove();
  ov = document.createElement('div');
  ov.id = 'copyWorkoutConfirmOverlay';
  ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);display:flex;align-items:flex-end;justify-content:center;z-index:200;';
  ov.innerHTML = `
    <div style="background:var(--surface);border-radius:16px 16px 0 0;padding:20px;width:100%;max-width:480px;">
      <div style="font-family:var(--font-display);font-weight:600;font-size:16px;margin-bottom:8px;">Replace what's entered so far?</div>
      <div style="font-size:13px;color:var(--text-dim);margin-bottom:16px;">You've already entered some numbers today — copying last time's workout (${formatAU(last.date)}) will replace them.</div>
      <div style="display:flex;gap:10px;">
        <button class="btn secondary" id="copyWorkoutCancelBtn" style="flex:1;">Cancel</button>
        <button class="btn" id="copyWorkoutConfirmBtn" style="flex:1;">Replace</button>
      </div>
    </div>
  `;
  document.body.appendChild(ov);
  document.getElementById('copyWorkoutCancelBtn').onclick = ()=>ov.remove();
  document.getElementById('copyWorkoutConfirmBtn').onclick = ()=>{
    ov.remove();
    applyCopiedWorkout(main, last);
  };
}

function missingProgramExercises(day){
  if(!day || day.type !== 'lift') return [];
  const loggedKeys = new Set(logState.exercises.map(e=>canonicalKey(e.name)));
  return (day.exercises||[]).filter(pe=>!loggedKeys.has(canonicalKey(pe.name)));
}
function openAddRemainingFlow(main){
  const day = plan[logState.dayKey];
  const missing = missingProgramExercises(day);
  ...
  ov.innerHTML = `
    <div style="background:var(--surface);border-radius:16px 16px 0 0;padding:20px;width:100%;max-width:480px;max-height:80vh;overflow-y:auto;">
      <div style="font-family:var(--font-display);font-weight:600;font-size:16px;margin-bottom:6px;">Add remaining exercises</div>
      <div style="font-size:12px;color:var(--text-dim);margin-bottom:14px;">Tick the ones you want to add — nothing gets added unless you tick it.</div>
      ${missing.map((pe,i)=>`
        <label style="display:flex;align-items:center;gap:10px;padding:10px 0;border-top:${i>0?'1px solid var(--border)':'none'};">
          <input type="checkbox" class="remainingExCheck" data-idx="${i}" checked style="width:18px;height:18px;flex-shrink:0;">
          <span>
            <div style="font-family:var(--font-display);font-weight:600;font-size:14px;">${escapeHtml(pe.name)}</div>
            ${pe.target ? `<div class="eyebrow">${escapeHtml(pe.target)}</div>` : ''}
          </span>
        </label>
      `).join('')}
      <div style="display:flex;gap:10px;margin-top:16px;">
        <button class="btn secondary" id="remainingCancelBtn" style="flex:1;">Cancel</button>
        <button class="btn" id="remainingAddBtn" style="flex:1;">Add ticked</button>
      </div>
    </div>
  `;
  document.body.appendChild(ov);
  document.getElementById('remainingCancelBtn').onclick = ()=>ov.remove();
  document.getElementById('remainingAddBtn').onclick = ()=>{
    ov.querySelectorAll('.remainingExCheck:checked').forEach(cb=>{
      const pe = missing[+cb.dataset.idx];
      logState.exercises.push({ id: uid(), name: pe.name, target: pe.target||'', videoUrl: pe.videoUrl||'', sets: presetSets(parseSetCountFromTarget(pe.target)) });
    });
    ov.remove();
    renderLog(main);
  };
...
async function handleSaveSession(){
  ...
  const isCardioDay = dayInfo && dayInfo.type === 'cardio';
  const exercisesToSave = logState.exercises
    .map(e=>({ name: e.name, target: e.target, sets: e.sets.filter(s=>s.weight!=='' || s.reps!=='') }))
    .filter(e=>e.sets.length > 0);
  const cardioNoteTrimmed = (logState.cardioNote||'').trim();
  const errEl = document.getElementById('logSaveErr');
  if(exercisesToSave.length === 0 && !(isCardioDay && cardioNoteTrimmed)){
    if(errEl){
      errEl.textContent = isCardioDay
        ? "Nothing saved — add a cardio note, then tap Save session again."
        : "Nothing saved — enter at least a weight or reps for one exercise, then tap Save session again.";
      errEl.style.display = 'block';
      errEl.scrollIntoView({ behavior:'smooth', block:'center' });
    }
    showToast(isCardioDay ? 'Add a cardio note before saving' : 'Log at least one set before saving');
    return;
  }
  if(errEl) errEl.style.display = 'none';
  // CONFIRMED: sessions(client_id, date, day_key, day_label, exercises jsonb, note, cardio jsonb)
  const { error } = await supabaseClient.from('sessions').upsert({
    ...
    note: logState.note,
    cardio: cardioNoteTrimmed ? { note: cardioNoteTrimmed } : null,
...
function renderSessionCardHtml(s){
  ...
  if(open){
    ...
    if(s.cardio && s.cardio.note) body += `<div style="font-size:13px;margin-bottom:8px;"><span style="color:var(--text-faint);">Cardio: </span>${escapeHtml(s.cardio.note)}</div>`;
    (s.exercises||[]).forEach(ex=>{
      ...
      (ex.sets||[]).forEach(st=>{ body += `<span class="chip">${escapeHtml(st.weight)||'–'}kg × ${escapeHtml(st.reps)||'–'}${st.rir?' @ RIR '+escapeHtml(st.rir):''}</span>`; });
      body += `</div>`;
    });
...
function renderProgram(main){
  ...
  const clinicianToolbar = `<div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:14px;">
    <button id="editFullProgramBtn" class="btn secondary" style="flex:1;min-width:140px;">Edit full program</button>
    <button id="importNotesBtn" class="btn secondary" style="flex:1;min-width:140px;">Import from notes</button>
  </div>`;
...
function goToLogForDate(iso, dayName){
  ...
  if(existing){
    ...
    logState.cardioNote = (existing.cardio && existing.cardio.note) || '';
    logState._initedForPlan = true;
  } else {
    logState.cardioNote = '';
  }
  ...
  main.innerHTML = `
    ...
    <div class="card" style="margin-top:10px;">
      <div class="field" style="margin-bottom:0;"><label>Block start date</label>
        <div class="dateinput-wrap" style="max-width:200px;"><input type="date" id="startDateInput" value="${settings.startDate}" style="width:auto;"></div>
      </div>
    </div>
...
function renderProgress(main){
  ...
  main.innerHTML = `<div style="padding:16px 18px;">
    ...
    <div style="display:flex;gap:10px;margin:14px 0;">
      <div class="card" style="flex:1;min-width:0;"><div class="eyebrow">Latest top set</div><div style="font-family:var(--font-display);font-weight:700;font-size:18px;">${latest!==null?latest+'kg':'—'}</div></div>
      <div class="card" style="flex:1;min-width:0;"><div class="eyebrow">Best logged</div><div style="font-family:var(--font-display);font-weight:700;font-size:18px;color:var(--brass);">${best!==null?best+'kg':'—'}</div></div>
    </div>
    ${goalHtml}
  ...
  heroGroups.forEach((label, key)=>{
    ...
    div.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;gap:10px;">
        <div style="font-family:var(--font-display);font-weight:600;font-size:15px;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(label)}</div>
...
function renderMeasure(main){
  ...
  html += `<div class="card" style="margin-bottom:14px;">
    ...
    <div class="field"><label>Date</label><div class="dateinput-wrap"><input type="date" id="measureDate" value="${d.date}"></div></div>
...
function renderCycle(main){
  ...
  let html = `<div style="padding:16px 18px;">
  <div class="field"><label>Date</label><div class="dateinput-wrap"><input type="date" id="cycleDateInput" value="${cycleState.date}"></div></div>
