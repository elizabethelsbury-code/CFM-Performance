...
async function loadSettings(){
  const { data, error } = await supabase.from('settings').select('*').eq('client_id', currentUser.id).maybeSingle();
...
async function saveStartDate(newDate){
  const { error } = await supabase.from('settings').upsert({ client_id: currentUser.id, start_date: newDate }, { onConflict: 'client_id' });
  if(error){ showToast("Couldn't save — " + error.message); return; }
...
async function loadCycleEntries(){
  const { data, error } = await supabase.from('cycle_entries').select('*').eq('client_id', currentUser.id).order('date', { ascending: false });
...
function wireCycleView(main){
  ...
  document.getElementById('saveCycleBtn').onclick = async ()=>{
    ...
    const { error } = await supabase.from('cycle_entries').upsert(row, { onConflict: 'client_id,date' });
    if(error){ showToast("Couldn't save — " + error.message); return; }
